...
if not sys.stdin.isatty():
    # Avoid GUI requirements when running non-interactively.
    matplotlib.use("Agg")
...
data = pd.read_csv(os.path.join(BASE_DIR, "AI-Data.csv"))
print(f"Loaded AI-Data.csv: {data.shape[0]} rows, {data.shape[1]} columns")
...
if sys.stdin.isatty():
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(BASE_DIR, "correlation_heatmap.png"), dpi=200)
    plt.close()
# -----------------------------------------
NON_INTERACTIVE = not sys.stdin.isatty()
ch = 10 if NON_INTERACTIVE else 0
if NON_INTERACTIVE:
    print("Non-interactive mode detected: skipping graphs menu.")
while(ch != 10):
...
for column in data.columns:
    # pandas 3 often infers columns as dtype 'str' instead of 'object'
    # so we treat both as categorical inputs needing label encoding.
    if is_object_dtype(data[column].dtype) or is_string_dtype(data[column].dtype):
        le = pp.LabelEncoder()
        data[column] = le.fit_transform(data[column])
        if column == "Class":
            class_le = le
        elif column == "StudentAbsenceDays":
            absence_le = le
...
choice = "n" if NON_INTERACTIVE else input("Do you want to test specific input (y or n): ")
if(choice.lower()=="y"):
    ...
    if absence_le is not None:
        absc = absence_le.transform([absc])[0]
    else:
        if (absc == "Under-7"):
           absc = 1
        elif (absc == "Above-7"):
    ...
    predD = modelD.predict(arr.reshape(1, -1))[0]
    predR = modelR.predict(arr.reshape(1, -1))[0]
    predP = modelP.predict(arr.reshape(1, -1))[0]
    predL = modelL.predict(arr.reshape(1, -1))[0]
    predN = modelN.predict(arr.reshape(1, -1))[0]

    if class_le is not None:
        predD = class_le.inverse_transform([int(predD)])[0]
        predR = class_le.inverse_transform([int(predR)])[0]
        predP = class_le.inverse_transform([int(predP)])[0]
        predL = class_le.inverse_transform([int(predL)])[0]
        predN = class_le.inverse_transform([int(predN)])[0]
    else:
        if (predD == 0):
            predD = "H"
        elif (predD == 1):
